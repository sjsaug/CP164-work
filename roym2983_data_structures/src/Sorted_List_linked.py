...
class Sorted_List:
    ...
    def insert(self, value):
        ...
        if self._front is None:
            ...
            n = _SL_Node(deepcopy(value), current)
            previous._next = n
        self._count += 1
        return None
    ...
    def index(self, key):
        ...
        # your code here
        _, __, i = self._linear_search(key)
        return i
                ...
                if curr is None:
                    if self._front is None:
                        ...
                        while curr._value <= value:
                            prev = curr
                            curr = curr._next
                        node = _SL_Node(deepcopy(value), curr)
                        prev._next = node
                    self._count += 1
            n1 = n1._next
        return None
    ...
    def union(self, source1, source2):
        ...
        while n1 is not None:
            value = n1._value
            prev, curr, _ = self._linear_search(value)
            if curr is None:
                if self._front is None:
                    node = _SL_Node(deepcopy(value), None)
                    self._front = node
    ...
    def __eq__(self, target):
        ...
        if self._count != target._count:
            ...
            tn = target._front
            sn = self._front
            while sn is not None and equals is True:
                if sn._value != tn._value:
                    equals = False
                sn = sn._next
                tn = tn._next
        return equals
    ...
    def combine(self, source1, source2):
        ...
        while source1_node is not None and source2_node is not None:
            if source1_node._value <= source2_node._value:
                if cn is None:
                    self._front = source1_node
                else:
                    cn._next = source1_node
                cn = source1_node
                source1_node = source1_node._next
            else:
                if cn is None:
                    self._front = source2_node
                else:
                    cn._next = source2_node 
                cn = source2_node
